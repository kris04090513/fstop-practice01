...
export class ItemService {
  ...
  get<T>(path: string) {
    ...
    return this.http
      ...
      .pipe(retry(1), timeout(60000), catchError(this.handleError));
  }
  ...
  getBoardItem() {
    let boardItemData = [
      { titleAcc: '臺幣帳戶', twCount: 922450, card: 'card1.png' },
  ...
  //   return chartItem;
  // }
  getChartItem(v:number) {
    let chartItem = [
      { title: `近${v}天收支`, bgc: '#F78EBD' },
      { title: `近${v}天轉入金額`, bgc: '#9197F2' },
      { title: `近${v}天轉出金額`, bgc: '#FEC133' },
      { title: `近${v}天利息`, bgc: '#8EFB99' },
    ];
    return chartItem;
  }
}
